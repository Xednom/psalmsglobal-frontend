  ...
  <div>
    <div class="card">
      <div class="card-header bg-transparent border-0">
        <h3 class="mb-0">Payment History and Converted minutes</h3>
      </div>
      <b-container fluid>
...
export default {
  ...
  computed: {
    ...mapGetters({
      paymentSummaries: "prepaid/paymentSummary/paymentSummaries",
      pagination: "prepaid/paymentSummary/paymentSummariesPagination",
      user: "user/user",
      client: "user/clientUser"
    }),
    subTotalAmountPaid: function() {
      return this.paymentSummaries.reduce((acc, item) => {
        return acc + parseInt(item.total_amount_paid);
      }, 0);
    },
    subTotalConvertedMins: function() {
      return this.paymentSummaries.reduce((acc, item) => {
        return acc + parseInt(item.total_converted_minutes);
      }, 0);
    },
  },
  data() {
    return {
      ...
      fields: [
        { key: "date_purchase", sortable: true },
        { key: "total_amount_paid", sortable: true },
        { key: "total_converted_minutes", sortable: true },
  ...
  methods: {
    ...
    async fetchPaymentSummaries() {
      this.isBusy = true;
      try {
        await this.$store
          .dispatch("prepaid/paymentSummary/fetchPaymentSummaries", this.pagination)
          .then(() => {
            this.totalRows = this.paymentSummaries.length;
            this.isBusy = false;
          });
      } catch (e) {
