        ...
        <card card-body-classes="table-full-width">
          <div>
            <div class="card-header bg-transparent border-0">
              <h3 class="mb-0">Active Plan Detail</h3>
            </div>
            <b-container fluid>
              <b-row>
              ...
              >
                ...
                <template #cell(account_status)="row">
                  <b-badge variant="success" v-if="row.item.account_status"
                    >active</b-badge
                  >
                  <b-badge variant="danger" v-else-if="!row.item.account_status"
                    >inactive</b-badge
                  >
                </template>

                <template #cell(recurring_bill)="row">
                  <b-badge variant="success" v-if="row.item.recurring_bill"
                    >active</b-badge
                  >
                  <b-badge variant="danger" v-else-if="!row.item.recurring_bill"
                    >inactive</b-badge
                  >
                </template>
...
export default {
  ...
  computed: {
     ...
     */
    ...mapGetters({
      activePlanDetails: "vodaconnect/activePlanDetail/activePlanDetails",
      pagination: "vodaconnect/activePlanDetail/activePlanDetailsPagination"
    }),
    queriedData() {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: "total_minutes_included", sortable: true },
  ...
  methods: {
    ...
    async fetchActivePlanDetails() {
      ...
      try {
        await this.$store
          .dispatch(
            "vodaconnect/activePlanDetail/fetchActivePlanDetails",
            this.pagination
          )
          .then(() => {
            this.totalRows = this.activePlanDetails.length;
            this.isBusy = false;
          });
      } catch (e) {
