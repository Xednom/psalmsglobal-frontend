  ...
  <div>
    <div class="card">
      <div class="card-header bg-transparent border-0">
        <h3 class="mb-0">Customer Registration Info</h3>
      </div>
      <b-container fluid>
    ...
    <!-- info modal -->
    <b-modal id="modal-view" :title="subscription.company" hide-footer>
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      pagination: "subscription/subscriptionsPagination",
      user: "user/user",
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: "signed_up_date", sortable: true },
        { key: "signed_out_date", sortable: true },
        { key: "billing_cycle", sortable: true },
        { key: "date_call_started", sortable: true },
  ...
  methods: {
    ...
    async fetchSubscriptions() {
      ...
      try {
        await this.$store
          .dispatch("subscription/fetchSubscriptions", this.pagination)
          .then(() => {
            this.isBusy = false;
    ...
    async fetchSubscription(subscription) {
      ...
      try {
        await this.$store
          .dispatch("subscription/fetchSubscription", subscription)
          .then(() => {
            this.isBusy = false;
          });
      } catch (e) {
