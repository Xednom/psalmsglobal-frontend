...
export const actions = {
  async fetchSubscriptions({ commit, dispatch }, params) {
    return await this.$axios
      .get("/api/v1/post-paid/subscription/", { params: params })
      .then(res => {
  ...
  async fetchSubscription({ commit }, payload) {
    return await this.$axios
      .get(`/api/v1/post-paid/subscription/${payload.id}/`)
      .then(res => {
  ...
  async saveMonthlyCharge({ commit }, payload) {
    let url = "/api/v1/post-paid/subscription/";
    try {
  ...
  async updateMonthlyCharge({ commit }, payload) {
    let url = `/api/v1/post-paid/subscription/${payload.id}/`;
    let method = "put";
