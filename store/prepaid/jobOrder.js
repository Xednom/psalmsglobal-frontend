  ...
  export const actions = {
    async fetchJobOrders({ commit, dispatch }, params) {
      return await this.$axios
        .get("/api/v1/prepaid/job-order-general/", { params: params })
        .then(res => {
    ...
    async fetchJobOrder({ commit, dispatch }, payload) {
      let endpoint = `/api/v1/prepaid/job-order/${payload}/`;
      return await this.$axios
        .get(endpoint)
        .then(res => {
    ...
    async savePrepaidJobOrder({ commit }, payload) {
      let url = "/api/v1/prepaid/job-order/";
      return await this.$axios.post(url, payload).then(() => {
        commit("setBasicField", payload);
      });
    },
    async updatePrepaidJobOrder({ commit }, payload) {
      let url = `/api/v1/prepaid/job-order-general/${payload.ticket_number}/`;
      let method = "put";
      return await this.$axios[method](url, payload).then(res => {
        return res.data;
      });
    },
    async addPrepaidComment({ commit }, payload) {
      let url = `/api/v1/prepaid/job-order/${payload.id}/comment/`;
